...
namespace HtmlParser
{
    class MakeTree
    {
        ...
        /// </summary>
        /// <returns></returns>
        private Tree turnToTree()
        {
            List<Tag> list = purify.format.rc.List;         //标签序列
            Tree tree = new Tree();           //树结构
            Stack<TreeNode> stack = new Stack<TreeNode>();        //存储节点的栈
            for(int i = 0; i < list.Count; i++)
            {
                ...
                if (type == 0 || type == 2)
                {
                    TreeNode node = new TreeNode() { node = tag };        //将该标签封装为树节点
                    if (stack.Count > 0)
                    {
                        node.parent = stack.Peek();         //该节点的父节点是栈顶元素

                        TreeNode temp = stack.Peek();      //取出栈顶元素
                        if (temp.childern == null)              //判断父节点的子节点序列是否为空
                        {
                            temp.childern = new List<TreeNode>();
                        }
